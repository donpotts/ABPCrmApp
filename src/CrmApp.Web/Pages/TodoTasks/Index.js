$(function () {
    ...
    var dataTable = $('#TodoTasksTable').DataTable(
        abp.libs.datatables.normalizeConfiguration({
            ...
            searching: false,
            scrollX: true,
            ajax: abp.libs.datatables.createAjax(crmApp.todoTasks.todoTask.getList),
            columnDefs: [
                {
                    title: l('Actions'),
                    rowAction: {
                        items:
                            [
                                {
                                    text: l('Edit'),
                                    visible: abp.auth.isGranted('CrmApp.TodoTasks.Edit'), //CHECK for the PERMISSION
                                ...
                                {
                                    text: l('Delete'),
                                    visible: abp.auth.isGranted('CrmApp.TodoTasks.Delete'), //CHECK for the PERMISSION
                                    confirmMessage: function (data) {
                                        return l('TodoTaskDeletionConfirmationMessage');
                                    },
                                    action: function (data) {
                                        crmApp.todoTasks.todoTask
