...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("CrmApp") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "CrmApp", DisplayName = "CRM App API", Resources = { "CrmApp" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["CrmApp_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["CrmApp_Web:RootUrl"]!.EnsureEndsWith('/');

            /* CrmApp_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["CrmApp_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["CrmApp_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["CrmApp_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // App Client
        var appClientId = configurationSection["CrmApp_App:ClientId"];
        if (!appClientId.IsNullOrWhiteSpace())
        {
                await CreateApplicationAsync(
                    ...
                    secret: configurationSection["CrmApp_App:ClientSecret"] ?? "1q2w3e*",
